...
export function getBlogPostHreflangLinks(bgSlug, hasTranslationFn, getTranslatedSlugFn) {
  const encodedBgSlug = encodeURIComponent(bgSlug);
  const languages = {
    'x-default': `${SITE_URL}/bg/blog/${encodedBgSlug}`,
    bg: `${SITE_URL}/bg/blog/${encodedBgSlug}`,
  };
  ...
  for (const locale of allLocales) {
    ...
    if (hasTranslationFn(bgSlug, locale, 'post')) {
      ...
      languages[locale] = `${SITE_URL}/${locale}/blog/${encodeURIComponent(translatedSlug)}`;
    }
  }
