...
/**
 * Normalizes a blog slug to a single decoded form (matches sitemap + crawled URLs).
 */
...
/**
 * Builds an absolute blog post URL using the same slug format everywhere
 * (canonical, hreflang, sitemap, schema). Uses decoded Unicode slugs so Cyrillic
 * URLs match what crawlers discover — avoids hreflang vs canonical mismatches.
 */
export function buildBlogPostUrl(locale, slug) {
  return `${SITE_URL}/${locale}/blog/${normalizeBlogSlug(slug)}`;
}
