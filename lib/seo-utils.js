...
/**
 * Relative blog post path for next-intl Link href (locale prefix added by router).
 */
export function getBlogPostHref(slug) {
  return `/blog/${encodeBlogSlug(slug)}`;
}

/**
 * Relative blog post path for redirects (includes locale prefix).
 */
