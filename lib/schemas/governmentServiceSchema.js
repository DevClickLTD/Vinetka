 ...
 */
export function getGovernmentServiceSchema(locale = 'bg', domain = 'avtovia') {
  const siteUrl = getSiteUrl(domain);
  const content = {
    bg: {
      name: "Информация за електронни винетки в България",
      description: "Информационна услуга за проверка и закупуване на електронни винетки в България. Синхронизирана с официалната система на АПИ.",
  ...
  return {
    ...
    "@id": `${siteUrl}/${locale}#governmentservice`,
    "name": data.name,
    ...
    "termsOfService": `${siteUrl}/${locale}/obshti-usloviya`,
 ...
 */
export function getGovernmentOrganizationsSchema(locale = 'bg', domain = 'avtovia') {
  const siteUrl = getSiteUrl(domain);
  return {
    "@context": "https://schema.org",
    "@type": "ItemList",
    ...
    "itemListElement": [
      {
        ...
        "item": {
          ...
          "description": locale === 'bg'
            ? "Агенция Пътна инфраструктура - официален оператор на електронните винетки в България"
 ...
 */
export function getVignetteCheckPublicServiceSchema(locale = 'bg', domain = 'avtovia') {
  const siteUrl = getSiteUrl(domain);
  return {
    "@context": "https://schema.org",
    "@type": "PublicService",
    "@id": `${siteUrl}/${locale}/proverka-na-vinetka#publicservice`,
    ...
    "availableChannel": {
      "@type": "ServiceChannel",
      "serviceUrl": `${siteUrl}/${locale}/proverka-na-vinetka`,
      "serviceType": "Online Service",
 ...
 */
export function getVignetteCheckSoftwareSchema(locale = 'bg', domain = 'avtovia') {
  const siteUrl = getSiteUrl(domain);
  const content = {
    bg: {
      name: "Проверка на винетка онлайн",
      description: "Безплатно уеб приложение за моментална проверка на валидността на електронни винетки в реално време. Синхронизирано с официалната база данни на АПИ.",
  ...
  return {
    ...
    "@id": `${siteUrl}/${locale}/proverka-na-vinetka#softwareapp`,
    "name": data.name,
    "description": data.description,
    "url": `${siteUrl}/${locale}/proverka-na-vinetka`,
    ...
    "screenshot": `${siteUrl}/default.webp`,
    "softwareHelp": {
      "@type": "CreativeWork",
      "url": `${siteUrl}/${locale}/proverka-na-vinetka`
    },
