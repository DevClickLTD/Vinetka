...
export function getHowToBuyVignetteSchema(locale = 'bg', domain = 'avtovia') {
 ...
 */
export function getHowToCheckVignetteSchema(locale = 'bg', domain = 'avtovia') {
  const siteUrl = getSiteUrl(domain);
  const content = {
    bg: {
      ...
      steps: [
        {
          ...
          text: "Посетете страницата за проверка на винетка на Avtovia.bg. Услугата е напълно безплатна и работи 24/7.",
      ...
      steps: [
        {
          ...
          text: "Visit the vignette check page on Avtovia.bg. The service is completely free and works 24/7.",
          url: `${siteUrl}/${locale}/proverka-na-vinetka`
        },
        {
  ...
  return {
    ...
    "step": data.steps.map((step, index) => ({
      ...
      "name": step.name,
      "text": step.text,
      "url": step.url || `${siteUrl}/${locale}/proverka-na-vinetka`,
      "itemListElement": step.tip ? [{
        "@type": "HowToTip",
        "text": step.tip
      }] : undefined
