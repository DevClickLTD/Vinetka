...
import { buildBlogPostUrl } from '../seo-utils';
import { getTranslatedSlug } from '../wordpress-helpers';

function getPostUrl(post, locale) {
  const slug = locale === 'bg'
    ? post.slug
    : (getTranslatedSlug(post.slug, locale, 'post') || post.slug);
  return buildBlogPostUrl(locale, slug);
}
 ...
 */
export function getBlogPostingSchema(post, locale = 'bg') {
 ...
 */
export function getBlogSchema(locale = 'bg') {
  const siteUrl = 'https://www.avtovia.bg';
  return {
    "@context": "https://schema.org",
    "@type": "Blog",
 ...
 */
export function getBlogListingSchema(posts, currentPage, totalPages, locale = 'bg') {
  const siteUrl = 'https://www.avtovia.bg';
  return {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
