...
export const vignetteTypes = {
  daily: {
    bg: {
      name: 'Дневна винетка',
      alternateName: ['1-дневна винетка', 'Daily vignette', 'Винетка за един ден'],
      description: 'Електронна винетка за леки автомобили до 3.5 тона, валидна 1 ден (24 часа) от момента на активиране. Идеална за еднодневни пътувания. Най-икономичната опция за кратки визити. Покрива всички платени магистрали в България.',
      sku: 'VIG-DAILY-1D',
      price: '8.00',
      priceEur: '4.09',
      duration: '1 ден',
    ...
    en: {
      name: 'Daily Vignette',
      alternateName: ['1-day vignette', 'One day vignette'],
      description: 'Electronic vignette for light vehicles up to 3.5 tons, valid for 1 day (24 hours) from activation. Perfect for one-day trips. Most economical option for short visits. Covers all paid highways in Bulgaria.',
      sku: 'VIG-DAILY-1D',
      price: '8.00',
      priceEur: '4.09',
      duration: '1 day',
 ...
 */
export function getVignetteProductSchema(type, locale = 'bg', domain = 'avtovia', options = {}) {
  ...
  return {
    ...
    "isRelatedTo": getRelatedProducts(type, locale, siteUrl),
    "potentialAction": {
      "@type": "BuyAction",
      "target": {
        "@type": "EntryPoint",
 ...
 */
function getRelatedProducts(currentType, locale, siteUrl) {
  const baseUrl = siteUrl;
 ...
 */
export function getVignettePriceListSchema(locale = 'bg', domain = 'avtovia') {
  const siteUrl = getSiteUrl(domain);
