...
export function getVignetteProductSchema(type, locale = 'bg', options = {}) {
  ...
  const baseUrl = options.baseUrl || 'https://vinetka.bg';
 ...
 */
export function getVignettePriceListSchema(locale = 'bg', baseUrl = 'https://vinetka.bg') {
  const types = ['weekend', 'weekly', 'monthly', 'quarterly', 'annual'];
  
  // Примерни reviews за всеки тип винетка (трябва да се заменят с реални при внедряване)
  const sampleReviews = {
    weekend: [
      {
        rating: "5",
        author: locale === 'bg' ? "Георги С." : "Georgi S.",
        body: locale === 'bg' 
          ? "Идеална за кратко пътуване. Активира се моментално!" 
          : "Perfect for a short trip. Activates instantly!",
        date: "2024-12-15"
      }
    ],
    weekly: [
      {
        rating: "5",
        author: locale === 'bg' ? "Мария И." : "Maria I.",
        body: locale === 'bg' 
          ? "Отлично съотношение цена-качество. Препоръчвам!" 
    ...
    monthly: [
      {
        rating: "5",
        author: locale === 'bg' ? "Иван П." : "Ivan P.",
        body: locale === 'bg' 
          ? "Перфектна за честите ми пътувания. Много удобна!" 
          : "Perfect for my frequent trips. Very convenient!",
        date: "2024-12-10"
      }
    ],
    quarterly: [
      {
        rating: "5",
        author: locale === 'bg' ? "Елена Д." : "Elena D.",
        body: locale === 'bg' 
          ? "Икономично решение за сезона. Много съм доволна!" 
          : "Economical solution for the season. Very satisfied!",
        date: "2024-12-08"
      }
    ],
    annual: [
      {
        rating: "5",
        author: locale === 'bg' ? "Петър К." : "Petar K.",
        body: locale === 'bg' 
          ? "Най-добрата опция! Спестявам много и нямам грижи." 
          : "The best option! I save a lot and have no worries.",
        date: "2024-12-05"
      }
    ]
  };
  ...
  return {
    ...
    "itemListElement": types.map((type, index) => {
      const data = vignetteTypes[type][locale] || vignetteTypes[type].bg;
      const reviews = sampleReviews[type] || [];
